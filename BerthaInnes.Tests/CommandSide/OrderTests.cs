...
namespace BerthaInnes.Domain.Tests.CommandSide
{
    public class OrderTests
    {
        ...
        {
            var domainEvents = Order.Decide(new StartOrder(new OrderId("1"), new List<Colis>()),
        ...
        {
            var domainEvents = Order.Decide(new TakeMarchandise(new OrderId("1"), new List<Colis>()),
                new List<IDomainEvent>());
            Assert.Empty(domainEvents);
        }
        ...
        [Fact]
        public void
            Given_OrderOf7Colis_With_5ColisReceived_When_TakeMarchandiseWith2Colis_Then_Raise_MarchandiseReceived()
        {
            var events = new List<IDomainEvent>
            {
                new OrderStarted(new OrderId("1"), 7), new MarchandisePartiallyReceived(new OrderId("1"), 2)
            };
        ...
        [Fact]
        public void
            Given_OrderStartedOf7Colis_With_3ColisReceived_When_TakeMarchandise_With_2Colis_Then_Raise_MarchandisePartiallyReceived()
        {
            var events = new List<IDomainEvent>
            {
                new OrderStarted(new OrderId("1"), 7), new MarchandisePartiallyReceived(new OrderId("1"), 4)
            };
            ...
            Assert.Equal(new MarchandisePartiallyReceived(new OrderId("1"), 2),
                (MarchandisePartiallyReceived)domainEvents.First());
        }
